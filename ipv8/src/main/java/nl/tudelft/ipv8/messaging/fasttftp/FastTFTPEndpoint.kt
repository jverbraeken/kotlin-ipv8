...
import nl.tudelft.ipv8.messaging.fasttftp.FastTFTPEndpoint.Companion.PREFIX_FASTTFTP
 ...
 *
 * All packets are prefixed with a [PREFIX_FASTTFTP] byte to allow it to be distinguished from regular
 * IPv8 UDP packets in UDPEndpoint which are prefixed with [Community.PREFIX_IPV8].
 */
class FastTFTPEndpoint(
    private val tftpClient: FastTFTPClient = FastTFTPClient()
) : Endpoint<IPv4Address>() {
    private val tftpSocket: TFTPSocket = TFTPSocket()
    private var busy = false
    internal var tftpServer = FastTFTPServer { packet ->
        scope.launch(Dispatchers.IO) {
            val datagram = packet.newDatagram()
            val wrappedData = byteArrayOf() + PREFIX_FASTTFTP + datagram.data
            datagram.setData(wrappedData, 0, datagram.length + 1)
            val socket = socket
            if (socket != null) {
                logger.debug {
                    "Send FastTFTP packet of type ${packet.type} to client " +
                        "${packet.address.hostName}:${packet.port} (${datagram.length} B)"
                }
                socket.send(datagram)
            } else {
                logger.error { "FastTFTP socket is missing" }
            }
        }
    }
    ...
    init {
        tftpServer.onFileReceived = { data, address, port ->
            ...
            logger.debug(
                "Received FastTFTP file (${data.size} B) from $sourceAddress"
            )
    ...
    override fun send(peer: IPv4Address, data: ByteArray) {
        if (busy) logger.debug { "Already busy => not sending data" }
        busy = true
...
//            logger.debug {
//                "Received FastTFTP packet of type ${tftpPacket.type} (${packet.length} B) " +
...
//            }
    ...
    inner class TFTPSocket : DatagramSocket() {
        ...
        override fun send(packet: DatagramPacket) {
            ...
            val wrappedData = byteArrayOf(PREFIX_FASTTFTP) + data
    ...
    companion object {
        ...
        const val PREFIX_FASTTFTP: Byte = 68
    }
}
