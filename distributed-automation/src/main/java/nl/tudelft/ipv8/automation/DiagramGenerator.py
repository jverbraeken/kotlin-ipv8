...
pdf2 = pd.read_csv("C:/Users/jverb/Downloads/evaluations/parsed - evaluation-simulated-2021-03-14_23.07.21.csv")
...
diagram_names = [
    ...
    "CIFAR-10 - 10 nodes + 4 noise attackers",
    "WISDM - 10 nodes + 4 noise attackers",

    "MNIST - 10 nodes + 4 2-label-flip attackers",
    "MNIST - 10 nodes + 4 all-label-flip attackers",
    "MNIST - 10 nodes + 2 Fang (2020) KRUM attackers",
    "MNIST - 10 nodes + 2 Fang (2020) Trimmed Mean attackers",

    "MNIST - 10 nodes non-i.i.d. + 4 2-label-flip attackers",
    "MNIST - 10 nodes non-i.i.d. + 4 all-label-flip attackers",
    "MNIST - 10 nodes non-i.i.d. + 2 Fang (2020) KRUM attackers",
    "MNIST - 10 nodes non-i.i.d. + 2 Fang (2020) Trimmed Mean attackers",

    "MNIST - 10 nodes non-i.i.d. + 2 noise attackers",
    "MNIST - 10 nodes non-i.i.d. + 4 noise attackers",
    "MNIST - 10 nodes non-i.i.d. + 8 noise attackers",
]
plt_regular_data = []
...
for (diagram, data) in enumerate(plt_regular_data_all):
    for (a, b, c, d, e) in data:
        plt.plot(a, b, label=c, linewidth=d, color=colors[e])
    if diagram > 2:
    ...
    # plt.show()
    plt.savefig("C:/Users/jverb/Downloads/evaluations/" + data[-1][2].split(" - ")[0] + " - regular.png")
    plt.clf()
...
for (diagram, data) in enumerate(plt_transfer_data_all):
    ...
    if diagram > 2:
        print(data[-1][2].split(" - ")[0] + " => " + diagram_names[diagram])
        bound = plt_bound_data[" Figure MinBound " + diagram_names[diagram].split(" - ")[0] + " _ " + ("noniid" if "non-i.i.d." in diagram_names[diagram] else "full")]
        plt.plot(bound["transfer"][0], bound["transfer"][1], ":", label="Min bound", linewidth=2, color="black")
        bound = plt_bound_data[" Figure MaxBound " + diagram_names[diagram].split(" - ")[0] + " _ " + ("noniid" if "non-i.i.d." in diagram_names[diagram] else "full")]
        plt.plot(bound["transfer"][0], bound["transfer"][1], "--", label="Max bound", linewidth=1, color="black")
    plt.xlim(0, 300)
    plt.ylim(0, 1)
    plt.legend()
    plt.title(diagram_names[diagram])
    # plt.show()
    plt.savefig("C:/Users/jverb/Downloads/evaluations/" + data[-1][2].split(" - ")[0] + " - transfer.png")
    plt.clf()
